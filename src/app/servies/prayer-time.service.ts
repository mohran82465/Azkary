...
export class PrayerTimeService {
  ...
  getNextPrayerTime(latitude: number, longitude: number): Observable<any> {
    const date = this.getToday();
    const timeZone = this.getTimeZone();
  
    const url = `https://api.aladhan.com/v1/nextPrayer/${date}?latitude=${latitude}&longitude=${longitude}&method=3&shafaq=general&tune=5,3,5,7,9,-1,0,8,-6&timezonestring=${timeZone}&calendarMethod=UAQ`;
